...
def get_af2_pdbs(uniprot_ids: tuple[str]) -> str:
    uniprot_id_objs: tuple[UniProtID] = tuple(
        UniProtID(uniprot_id=_id, uniprot_type="AF2", chain_id="A")
...
def test_plddt_masked_SA(uniprot_ids):
    ...
    parsed_seqs = FoldSeek(foldseek, plddt_threshold=70).parallel_queries(
        pdb_files=files, enable_plddt_masks=True
    )
