...
class TMalignSetup:
    ...
    def __init__(
        self,
        binary_path: str = "./bin/TMalign",
        source_url: str = "https://zhanggroup.org/TM-align/TMalign.cpp",
    ):
        """
    ...
    def fetch_source_code(self) -> None:
        ...
        if response.status_code != 200:
            raise RuntimeError(
                f"Failed to fetch source code from {self.source_url}"
            )
    ...
    def modify_source_code(self) -> None:
        ...
        if platform.system() == "Darwin":  # macOS
            modified_source_code = source_code.replace(
    ...
    def compile_source_code(self) -> None:
        ...
        ):  # Only add -static flag for non-macOS systems
        ...
        if result.returncode != 0:
            raise RuntimeError(
                f"Compilation failed: {result.stderr.decode('utf-8')}"
            )
...
class TMalign:
    ...
    def __init__(self, binary_path: str = "./bin/TMalign"):
        ...
        if not os.path.isfile(self.binary_path):
            raise RuntimeError(
                f"TMalign binary not found at {self.binary_path}"
            )
    ...
    def align(
        ...
        stick_align: bool = False,
        output_superposition: Optional[str] = None,
        normalize_length: Optional[int] = None,
        scale_d0: Optional[float] = None,
        output_matrix: Optional[str] = None,
        based_on_sequence: bool = False,
    ) -> str:
        """
        ...
        if result.returncode != 0:
            raise RuntimeError(
                f"TMalign failed with error: {result.stderr.decode('utf-8')}"
            )
