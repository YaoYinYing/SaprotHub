...
class StructureDownloader:
    ...
    def process_url(self, data_id: UniProtID) -> str:
        ...
        """
        if data_id.uniprot_type == "PDB" and (
            self.data_type == "plddt" or self.data_type == "pae"
        ):
    ...
    def run(self, payload: list[UniProtID]):
        ...
        return Parallel(n_jobs=self.nproc)(
            delayed(self.process_url)(data_id)
            for data_id in track(payload, description="Downloading...")
        )
