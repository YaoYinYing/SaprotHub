...
def get_peft_model_state_dict(
    ...
    if save_embedding_layers and hasattr(model, "get_input_embeddings"):
        for layer in [
            ...
            if (
                not is_embedding_in_target_modules
                or has_valid_embedding_base_layer(layer)
            ):
...
def set_peft_model_state_dict(
    ...
    for key, value in peft_model_state_dict.items():
        ...
        ), f"Key '{key}' not found in model state dict"
        assert (
            model.state_dict()[key].shape == value.shape
        ), f"Shape mismatch for key '{key}', expected {model.state_dict()[key].shape}, got {value.shape}"
