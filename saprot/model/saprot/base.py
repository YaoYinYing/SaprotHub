...
class SaprotBaseModel(AbstractModel):
    ...
    def _init_lora(self):
        ...
        assert self.lora_kwargs.num_lora >= len(config_list), (
            "The number of LoRA models should be greater than or "
            "equal to the number of weight files."
        )
        for i in range(self.lora_kwargs.num_lora):
            adapter_name = (
                f"adapter_{i}" if self.lora_kwargs.num_lora > 1 else "default"
            )
            ...
            if i < len(config_list):
                ...
                if i == 0:
                    ...
                    self.model = (
                        PeftModelForSequenceClassification.from_pretrained(
                            model=self.model,
            ...
            # Initialize LoRA model for training
                ...
                if i == 0:
                    ...
                    self.model = get_peft_model(
                        self.model, lora_config, adapter_name=adapter_name
                    )
        ...
        if self.lora_kwargs.num_lora > 1:
            ...
            print(
                "Multiple LoRA models are used. This only supports inference mode. If you want to train the model,"
                "set num_lora to 1."
            )
    ...
    def initialize_model(self):
        ...
        if self.task == "token_classification":
            ...
            if isinstance(self.model, EsmForMaskedLM) or isinstance(
                self.model, EsmForSequenceClassification
            ):
        ...
        if isinstance(self.model, EsmForMaskedLM) or isinstance(
            ...
            self.model.esm.encoder.gradient_checkpointing = (
    ...
    def get_hidden_states_from_dict(
        self, inputs: dict, reduction: str = None
    ) -> list:
        """
    ...
    def get_hidden_states_from_seqs(
        self, seqs: list, reduction: str = None
    ) -> list:
        """
    ...
    def add_bias_feature(self, inputs, coords: list[dict]) -> torch.Tensor:
        ...
        """
        inputs["pair_feature"] = batch_coords2feature(
            coords, self.model.device
        )
        return inputs
    ...
    def save_checkpoint(
        ...
        if not self.lora_kwargs:
            return super().save_checkpoint(
                save_path, save_info, save_weights_only
            )
        ...
        with self.grid.output_to(0, 0):
            ...
            for idx, metric in enumerate(log_dict.keys()):
                if metric in self.valid_metrics_list:
                    self.valid_metrics_list[metric].append(
                        log_dict[metric].detach().cpu().item()
                    )
                else:
                    self.valid_metrics_list[metric] = [
                        log_dict[metric].detach().cpu().item()
                    ]
