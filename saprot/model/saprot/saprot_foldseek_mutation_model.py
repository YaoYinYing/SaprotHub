...
class SaprotFoldseekMutationModel(SaprotBaseModel):
    ...
    def get_struc_seq(
        self, structure_content, structure_type, plddt, struc_seq
    ):
    ...
    def forward(
        ...
        if getattr(self, "struc_seq", None) is None:
            self.struc_seq = self.get_struc_seq(
                structure_content, structure_type, plddt, struc_seq
            )
        ...
        if getattr(self, "wild_type", None) is None:
            ...
            if self.seq_substitute_rate is not None:
                ...
                indices = np.random.choice(
                    len(wild_type),
                    int(len(wild_type) * self.seq_substitute_rate),
                    replace=False,
                )
        ...
        for i, (seq, info) in enumerate(zip(seqs, mut_info)):
            ...
            ori_seq = [
                a + b.lower() for a, b in zip(self.wild_type, self.struc_seq)
            ]
            ...
            for single in info.split(":"):
                ...
                if single[0] in aa_set:
                    ...
                    ori_seq[pos - ins_num - 1] = (
                        "#" + ori_seq[pos - ins_num - 1][-1]
                    )
        ...
        if len(ins_seqs) > 0:
            ins_inputs = self.tokenizer.batch_encode_plus(
                ins_seqs, return_tensors="pt", padding=True
            )
        ...
        ori_inputs = self.tokenizer.batch_encode_plus(
            ori_seqs, return_tensors="pt", padding=True
        )
        ...
        for i, data_list in enumerate(mut_data):
            ...
            for data in data_list:
                ...
                ori_st = self.tokenizer.get_vocab()[
                    ori_aa + foldseek_struc_vocab[0]
                ]
                mut_st = self.tokenizer.get_vocab()[
                    mut_aa + foldseek_struc_vocab[0]
                ]

                ori_prob = ori_probs[
                    i, ori_pos, ori_st : ori_st + len(foldseek_struc_vocab)
                ].sum()
                ...
                if i in ins_dict:
                    mut_prob = ins_probs[
                        ins_dict[i],
                        ins_pos,
                        mut_st : mut_st + len(foldseek_struc_vocab),
                    ].sum()
                    # mut_prob = ins_probs[ins_dict[i], ins_pos, mut_st: mut_st + len(foldseek_struc_vocab)].max()
                else:
                    mut_prob = ori_probs[
                        i, ori_pos, mut_st : mut_st + len(foldseek_struc_vocab)
                    ].sum()
                ...
                if self.MSA_log_path is not None and st <= ori_pos - 1 < ed:
                    ori_msa_prob = MSA_log_prior[
                        ori_pos - 1 - st, aa2id[ori_aa]
                    ]
                    mut_msa_prob = MSA_log_prior[
                        ori_pos - 1 - st, aa2id[mut_aa]
                    ]
                    pred += 0.4 * torch.log(mut_prob / ori_prob) + 0.6 * (
                        mut_msa_prob - ori_msa_prob
                    )
    ...
    def predict_mut(self, seq: str, mut_info: str) -> float:
        ...
        with torch.no_grad():
            ...
            for single in mut_info.split(":"):
                ...
                ori_prob = probs[
                    0, pos, ori_st : ori_st + len(foldseek_struc_vocab)
                ].sum()
                mut_prob = probs[
                    0, pos, mut_st : mut_st + len(foldseek_struc_vocab)
                ].sum()
    ...
    def predict_pos_mut(self, seq: str, pos: int) -> dict:
        ...
        with torch.no_grad():
            ...
            for mut_aa in aa_list:
                ...
                ori_prob = probs[
                    ori_st : ori_st + len(foldseek_struc_vocab)
                ].sum()
                mut_prob = probs[
                    mut_st : mut_st + len(foldseek_struc_vocab)
                ].sum()
