...
class SaProtIFModel(SaprotBaseModel):
    ...
    def on_train_epoch_end(self):
        ...
        if (
            getattr(
                self.trainer.datamodule.train_dataset, "num_per_cluster", None
            )
            is not None
        ):
    ...
    def on_test_epoch_end(self):
        ...
        log_dict["test_loss"] = torch.cat(
            self.all_gather(self.test_outputs), dim=-1
        ).mean()
    ...
    def on_validation_epoch_end(self):
        ...
        log_dict["valid_loss"] = torch.cat(
            self.all_gather(self.valid_outputs), dim=-1
        ).mean()
    ...
    def predict(
        ...
        if not len(aa_seq) == len(struc_seq):
            raise ValueError(
                f"The length of the amino acid sequence({len(aa_seq)=}) and the foldseek sequence ({len(struc_seq)=})must be the same. "
            )
        if not method in ["argmax", "multinomial"]:
            raise ValueError(
                "The prediction method must be either 'argmax' or 'multinomial'."
            )
        if method == "argmax" and num_samples == 1:
            raise ValueError(
                "The sample number must be 1 when the prediction method is 'argmax'."
            )

        sa_seq = "".join(
            f"{aa}{struc}" for aa, struc in zip(aa_seq, struc_seq.lower())
        )
        print(f"{sa_seq=}")
