...
def pdb2feature(pdb_path: str, device: str = "cpu"):
    ...
    pair_feature = torch.cat(
        [pair_feature, make_orientations(coords, device)], dim=-1
    )
...
def dist2feature(dist, device: str = "cpu"):
    dist = dist.to(device)
    pair_feature = torch.cat(
        [rbf(dist[..., i], 2.0, 22.0, 16) for i in range(dist.shape[-1])],
        dim=-1,
    )
...
def get_aligned_feature(
    mut_seq: str, wt_seq: str, wt_feature: torch.Tensor, align: bool = True
):
    if align:
