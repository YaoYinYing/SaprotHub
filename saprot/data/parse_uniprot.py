...
def extract_texts(data_dict: dict):
    ...
    for comment in data_dict.get("comments", []):
        ...
        if comment["commentType"] == "CATALYTIC ACTIVITY":
            ...
            physiologicalReactions = (
                comment["physiologicalReactions"][0]["directionType"]
                if "physiologicalReactions" in comment
                else "None"
            )
            note = f"{sep}".join(
                [
        ...
        if comment["commentType"] == "COFACTOR":
            for cofactor in comment.get("cofactors", []):
                ...
                molecule = (
                    comment["molecule"] if "molecule" in comment else "None"
                )
                note = f"{sep}".join(["molecule", molecule])
        ...
        if comment["commentType"] == "SUBCELLULAR LOCATION":
            for location in comment.get("subcellularLocations", []):
                ...
                topology = (
                    ...
                    if location.get("topology", None) is not None
                    else "None"
                )
                note = f"{sep}".join(
    ...
    for feature in data_dict.get("features", []):
        ...
        if feature["type"] == "Binding site":
            ...
            note = sep.join(["start", str(st), "end", str(ed)])
            ...
            for alt_name_dict in sub_dict:
                ...
                for short_name_dict in alt_name_dict.get("shortNames", []):
                    evidences = json.dumps(
                        short_name_dict.get("evidences", None)
                    )
                    text = short_name_dict["value"]
        ...
        if name == "includes":
            for includes in sub_dict:
                if "recommendedName" in includes:
                    ...
                    text = text_dict["value"]
                    note = sep.join(
                        ["includes", "recommendedName", "fullName"]
                    )
                    ...
                    # Short Names
                    for short_name_dict in includes["recommendedName"].get(
                        "shortNames", []
                    ):
                        evidences = json.dumps(
                            short_name_dict.get("evidences", None)
                        ...
                        text = short_name_dict["value"]
                        note = sep.join(
                            ["includes", "recommendedName", "shortNames"]
                        )
                ...
                for alt_name_dict in includes.get("alternativeNames", []):
                    ...
                    text = text_dict["value"]
                    note = sep.join(
                        ["includes", "alternativeNames", "fullName"]
                    )
                    ...
                    for short_name_dict in alt_name_dict.get("shortNames", []):
                        ...
                        text = short_name_dict["value"]
                        note = sep.join(
                            ["includes", "alternativeNames", "shortNames"]
                        )
        ...
        if name == "contains":
            for contain in sub_dict:
                if "recommendedName" in contain:
                    ...
                    text = text_dict["value"]
                    note = sep.join(
                        ["contains", "recommendedName", "fullName"]
                    )
                    ...
                    # Short Names
                    for short_name_dict in contain["recommendedName"].get(
                        "shortNames", []
                    ):
                        evidences = json.dumps(
                            short_name_dict.get("evidences", None)
                        ...
                        text = short_name_dict["value"]
                        note = sep.join(
                            ["contains", "recommendedName", "shortNames"]
                        )
                ...
                for alt_name_dict in contain.get("alternativeNames", []):
                    ...
                    text = text_dict["value"]
                    note = sep.join(
                        ["contains", "alternativeNames", "fullName"]
                    )
                    ...
                    for short_name_dict in alt_name_dict.get("shortNames", []):
                        ...
                        text = short_name_dict["value"]
                        note = sep.join(
                            ["contains", "alternativeNames", "shortNames"]
                        )
        ...
        for name, sub_dict in genes.items():
            ...
            if name == "orderedLocusNames":
                for orderedLocusName in sub_dict:
                    evidences = json.dumps(
                        orderedLocusName.get("evidences", None)
                    )
    ...
    if "organismHosts" in data_dict:
        ...
        for host in data_dict["organismHosts"]:
            ...
            note = sep.join(
                [
                    "scientificName",
                    host.get("scientificName", "None"),
                    "commonName",
                    host.get("commonName", "None"),
                    "synonyms",
                    host.get("synonyms", ["None"])[0],
                ]
            )
...
def record2text(
    ...
    def fill_template(template, template_id, extra_key=None):
        candidates = (
            ...
            else template[subsection][extra_key]
        )
    ...
    if subsection == "Gene names":
        if note == "orfNames" or note == "orderedLocusNames":
            ...
            text, used_template = fill_template(
                template, template_id, "normal"
            )
    ...
    if subsection == "Natural variant":
        ...
        if raw_text == "":
            text, used_template = fill_template(
                template, template_id, extra_key="without raw_text"
            )
        else:
            text, used_template = fill_template(
                template, template_id, extra_key="with raw_text"
            )
    ...
    if subsection == "Cross-link":
        ...
        if st == ed:
            text, used_template = fill_template(
                template, template_id, "interchain"
            )
        else:
            text, used_template = fill_template(
                template, template_id, "intrachain"
            )
