...
def make_dist_map(coords, eps=1e-6):
    ...
    CB = (
        -0.58273431 * vec_NtoCA
        + 0.56802827 * vec_CAtoC
        - 0.54067466 * vec_cross_product
        + coords[..., 1, :]
    )
    ...
    coords_dist = (
        ((coords_view1 - coords_view2).square().sum(dim=-1) + eps)
        .sqrt()
...
def make_rbf(coord_dict, num_rbf=16, device="cpu"):
    ...
    coords = torch.stack(
        [torch.tensor(coord_dict[k]) for k in ["N", "CA", "C", "O"]], dim=1
    )
    ...
    RBFs = torch.cat(
        [
            rbf(coords_dist[..., i], 2.0, 22.0, num_rbf)
            for i in range(coords_dist.shape[-1])
        ],
        dim=-1,
    )
...
def make_orientations(coord_dict, device="cuda"):
    ...
    O = torch.stack(
        (
            vec_CAtoN,
            vec_cross_product,
            torch.cross(vec_CAtoN, vec_cross_product),
        ),
        -2,
    )
    ...
    dU = torch.matmul(
        O[..., None, :, :].transpose(-1, -2), dX.unsqueeze(-1)
    ).squeeze(-1)
...
def global_to_local(R, t, q):
    ...
    p = torch.matmul(
        R[:, :, None, :].transpose(-1, -2), (q - t[:, :, None, :, None])
    )  # (N, L, 3, *)
    p = p.transpose(-1, -2).squeeze(
        dim=-2
    )  # (N, L, 3, *) -> (N, L, *, 3) -> (N, L, ..., 3)
...
def add_frame_noise(
    coords: torch.Tensor, mean, std, mask: torch.Tensor = None
):
    """
    ...
    noised_quats = noised_quats / torch.norm(
        noised_quats, dim=-1, keepdim=True
    )
...
def dict2tensor(dict_coords: dict, device="cpu"):
    ...
    """
    coords = torch.stack(
        [torch.tensor(dict_coords[k]) for k in ["N", "CA", "C", "O"]], dim=1
    )
    coords = coords.to(device)
